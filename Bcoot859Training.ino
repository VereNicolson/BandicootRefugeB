/**
  *  Bcoot859Training
  *  Purpose: Animals in proximity to an entrancee door will be detected by IR distance sensors. Inbound animals will cause initiation of an RFID scan. 
  *  Animals with chip numbers included in the sketch will be admitted, their PIT implant number will be recorded on an SD card, along with the time of the event.
  *  Proximity in the outbound direction will cause the door to be opened and the time recorded.  
  *  
  *    Version 859Training is a version amended from Bcoot859 to increment the amount of door closure slowly to allow animals to become accostomed to the door sound and movement
  *    If animals cease using door, rebooting will restart the training process with a fully open door.
  *    
  *    @Authur Vere Nicolson 09/12/2018
  *    @version 859t
  **/
...
#include "RTClib.h"        // Looks after clock, the Bcoot RTC is definitely DS1307
...
int DOOR_servoAngleShut = 1600;                       //global to share with door cycle and training. In this training version, door is initally same position when shut and decrements towards 950 in 65 steps of 10
...
int doorTight = HIGH;
int countDoorTries = 0;                             //  variable for limiting safety "re-cycles" to 3
int countDoorTrainingCycles = 0;                    // variable to couunt training cycles
...
void DOOR_trigger() {                                 //this function debugged and works including safety switch
  switch (DOOR_state) {
    ...
    case DOOR_STATE_INITIALISE:     //label 0
      ...
      DOOR_servoAngleShut = 1600 -(10 * countDoorTrainingCycles);
      if (DOOR_servoAngleShut < 950){
        DOOR_servoAngleShut = 950;
      }
    ...
    case DOOR_STATE_OPENING:                                      //label 1
      ...
      if ( millis() - DOOR_state_time > 500) {                                // allow 2sec for servo sweep
        ...
        countDoorTrainingCycles++ ;                 // setup initiates this at 0, increases each time door opens
      }
